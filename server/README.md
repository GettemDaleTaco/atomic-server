...
- Set `ATOMIC_LOG_FRAMES_DIR` or pass `--log-frames-dir` to store received blip / pip frames in files grouped by frame kind (`blip` or `pip`) and payload format (`json-ad`, `turtle`, `text`, `binary`).
