  ...
  async endToEnd(@argument() netlifyAuthToken: Secret): Promise<string> {
    ...
    const playwrightContainer = dag
      ...
      .withExec([
        ...
        `curl -fsSL https://get.pnpm.io/install.sh | env PNPM_VERSION=${PNPM_VERSION} ENV="$HOME/.shrc" SHELL="$(which sh)" sh - && export PATH=/root/.local/share/pnpm:$PATH && /bin/apt update && /bin/apt install -y zip`,
      ])
      .withEnvVariable(
        'PATH',
