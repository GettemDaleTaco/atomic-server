...
export function useDevDrive() {
  ...
  const createDevDrive = useCallback(async () => {
    ...
    try {
      ...
      // Personal first: `createDrive(..., { personal: false })` appends
      // the workspace to the home drive's saved-drives list, which only
      // exists after `linkPersonalDrive`. `agentName` is written on that
      ...
      // Secret `initialDrive` is home (personal), matching onboarding.
